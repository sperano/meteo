...
var (
	rootCmd = &cobra.Command{
		Use:   "bitmap2a2lr <source bitmap> <asm file to write>",
		Short: "Bitmap to Apple II Low-Res binary",
		Long: `Bitmap to Apple II Low-Res binary
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			for y := 0; y < 20; y++ {
				for x := 0; x < bounds.Dx(); x++ {
					...
					data = append(data, byte(pixel1|(pixel2<<4)))
				}
			}
